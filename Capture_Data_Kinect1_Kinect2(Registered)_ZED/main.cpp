...
//Delay time for ambient ilumination estabilization
//#include <unistd.h>
//usleep(int(5*1000000)); // wait 5 sec
...
string pathRGB, pathDepth, pathCloud;

int num_frames = 5; // Number of frames to capture
int count_save = 1;
char key = ' ';
...
void cvMat2pcd(cv::Mat cloudMat, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloudpcd)
{
    if( !cloudMat.empty())
    {
        for( int y = 0; y < cloudMat.rows; y++ ) 
        {
            ...
            {
                cv::Point3f pt_3d = cloudMat.at<cv::Vec3f>(y, x);
                pcl::PointXYZ point;
                point.x = pt_3d.x;
                point.y = pt_3d.y;
                point.z = pt_3d.z;
                cloudpcd->points.push_back(point);
            }
        }
        cloudpcd->width = (int) cloudpcd->points.size ();
        cloudpcd->height = 1;
    }
}
...
bool CaptureKv1()
{
    ...
    sensor1.set(CAP_OPENNI_DEPTH_GENERATOR_REGISTRATION, true);
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1pcd (new pcl::PointCloud<pcl::PointXYZ>);
            ...
            if (key == 's' || key == 'S')
            {
                ...
                for(int i = 1; i <= num_frames; i++)
                {
                    ...
                    {      
                        ...
                        sensor1.retrieve(cloud1, CAP_OPENNI_POINT_CLOUD_MAP); // XYZ in meters (CV_32FC3)
                        cvMat2pcd(cloud1, cloud1pcd);
                        pcl::io::savePCDFileASCII(pathCloud + to_string(count_save)+ "/" + to_string(i) + string(".pcd"), *cloud1pcd);
                        cloud1pcd->clear();
                    }
                    else
                    {  
...
bool CaptureKv2()
{
    ...
    libfreenect2::Frame depth_und(512, 424, 4), bgr_reg(512, 424, 4);

    cv::Mat bgrmatReg, depthmatUnd;

    new_width = bgr_reg.width;
    new_height = bgr_reg.height;
    ...
    while(key != 'q')
    {
        ...
        // rgb->format = libfreenect2::Frame::RGBX; //get image in RBGX fromat
        ...
        registration->apply(bgr, depth, &depth_und, &bgr_reg, true);
        listener.release(frames);

        cv::Mat(bgr_reg.height, bgr_reg.width, CV_8UC4, bgr_reg.data).copyTo(bgrmatReg);        
        flip(bgrmatReg, bgrmatReg, 1);
        cvtColor(bgrmatReg, bgrmatReg , COLOR_BGRA2BGR);
        imshow("RGB", bgrmatReg);

        //Only for depth visualization
        cv::Mat(depth_und.height, depth_und.width, CV_32FC1, depth_und.data).copyTo(depthmatUnd);
        flip(depthmatUnd, depthmatUnd, 1);    
        minMaxLoc(depthmatUnd, &minVal, &maxVal); //find minimum and maximum intensities
        depthmatUnd.convertTo(depthmatUnd, CV_8UC1, 255.0/(maxVal - minVal), -minVal * 255.0/(maxVal - minVal));
        imshow("Depth", depthmatUnd);       
        ...
        if (key == 's' || key == 's'){
            ...
            for(int i = 1; i <= num_frames; i++)
            {
                    ...
                    for (int r = 0; r < depth_und.height; r++)
                    {   
                        for (int c = 0; c < depth_und.width; c++)
                        {
                            registration->getPointXYZ(&depth_und, r, c, X, Y, Z);
                            pcl::PointXYZ point;
                            point.x = X;
                            point.y = Y;
                            point.z = Z;
                            cloud1pcd->points.push_back(point);
                        }
                    }
                    ...
                    flip(bgrmatReg, bgrmatReg, 1);
                    cvtColor(bgrmatReg, bgrmatReg, COLOR_BGRA2BGR);
                    imwrite(pathRGB + to_string(count_save)+ "/" + to_string(i) + string(".png"), bgrmatReg);   

                    //Save depth map on .png file 
                    cv::Mat(depth_und.height, depth_und.width, CV_32FC1, depth_und.data).copyTo(depthmatUnd);
                    depthmatUnd.convertTo(depthmatUnd, CV_16UC1);
                    flip(depthmatUnd, depthmatUnd, 1);
                    imwrite(pathDepth + to_string(count_save) + "/" + to_string(i) + string(".png"), depthmatUnd);                                                    
            
                    cloud1pcd->width = (int) cloud1pcd->points.size ();
                    cloud1pcd->height = 1;
                    pcl::io::savePCDFileASCII(pathCloud + to_string(count_save)+ "/" + to_string(i) + string(".pcd"), *cloud1pcd);
                    cloud1pcd->clear();            
            }
        ...
        // Recover BGRX frame
        // Mat bgrmat_read = imread("bgrmatReg.png", IMREAD_COLOR ); // read and convert image in 3 channel BGR.
        // flip(bgrmat_read, bgrmat_read, 1);
        // cvtColor(bgrmat_read, bgrmat_read, COLOR_BGR2BGRA);
        // libfreenect2::Frame bgrx_recover(bgrmat_read.cols, bgrmat_read.rows, 4, bgrmat_read.data);

        // FOR RECOVER DEPTH FRAME FROM DEPTH IMAGE
        // Mat depthmat_read = imread("depthmatUnd.png", IMREAD_ANYDEPTH);
        // flip(depthmat_read, depthmat_read, 1);
        // depthmat_read.convertTo(depthmat_read, CV_32FC1);
	    // libfreenect2::Frame depth_recover(depthmat_read.cols, depthmat_read.rows, 4, depthmat_read.data);

        // COMPROBATION
        // float x,y,z;
        // registration->getPointXYZ(&depth_und, 256, 212, x, y, z);
        // cout<<x<<" "<<y<<" "<<z<<endl;
        // registration->getPointXYZ(&depth_recover, 256, 212, x, y, z);
        // cout<<x<<" "<<y<<" "<<z<<endl;
...
{    
    ...
    //SELF_CALIBRATION_STATE state = zed.getSelfCalibrationState();
    //cout << "Self Calibration State: " << toString(state) << endl;
            ...
            if (key == 's' || key == 'S')
            {
                ...
                for(int i = 1; i <= num_frames; i++)
                {
                    if (sensor3.grab(runtime_parameters) == ERROR_CODE::SUCCESS) 
                    {
                        ...
                        //float max_value = std::numeric_limits<unsigned short int>::max(); # Only if save depth in METERS
                        //float scale_factor = max_value / sensor3.getDepthMaxRangeValue(); # Only if save depth in METERS
                        //saveDepthAs(depth_map, DEPTH_FORMAT_PNG, (pathDepth + to_string(count_save) + "/" +to_string(i)).c_str(),scale_factor);   
                        //saveDepthAs(sensor3, sl::DEPTH_FORMAT::PNG, (pathDepth + to_string(count_save) + "/" +to_string(i)).c_str());                
                        //savePointCloudAs(sensor3, sl::POINT_CLOUD_FORMAT::PCD_ASCII, (pathCloud + to_string(count_save) + "/" +to_string(i)).c_str(), true);
                        sl::Mat point_cloud;
                        sensor3.retrieveMeasure(point_cloud, sl::MEASURE::XYZRGBA);
                        point_cloud.write((pathCloud + to_string(count_save) + "/" +to_string(i)).c_str());
                    }
                }
                count_save++;
            }      
...
int main(int argc, char **argv)
{
    ...
    if (string(argv[1]) == sensor[0]) 
    {
        ...
        pathCloud = savedPath + "cloud_" + sensor[0]+ "/";
        if(CaptureKv1());       
    }
    else if (string(argv[1]) == sensor[1])
    {
        ...
        pathCloud = savedPath + "cloud_" + sensor[1]+ "/";
        if(CaptureKv2());
    }
    ...
    {   
        for (int i = 3; i < 7; i++)
        {
            ...
            pathCloud = savedPath + "cloud_" + sensor[i]+ "/";
